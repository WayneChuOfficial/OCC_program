...
class DetectTest:
    ...
    def select_box(self, predict, size, dims=[208,208], stride=16):
        wh = np.array([[size[0]], [size[1]]])
        probs = predict[0, :, :, 0:2]
        # # a = probs[:,:,1]>0.9
        # print(a)
        ...
        scale = [((dims[0] + 40.0) / 2.0) / stride,((dims[1] + 40.0) / 2.0) / stride]
        unit = np.array([[-0.5, -0.5, 1], [0.5, -0.5, 1], [0.5, 0.5, 1], [-0.5, 0.5, 1]]).transpose((1, 0))
        h, w, _ = probs.shape
        candidates = []
        for i in range(h):
            for j in range(w):
                if probs[i, j, 1] > self.confidence_threshold:
                    affine = affines[i, j]
                    pts = affine @ unit
                    # print(affine)
                    # print(affine)
